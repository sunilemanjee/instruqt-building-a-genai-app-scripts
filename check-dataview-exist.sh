...
# Script to check if an Elasticsearch dataview exists using embedded Python
...
DATAVIEW_NAME="serverless-demo-dataview"
echo "Checking if dataview '$DATAVIEW_NAME' exists..."
...
# Embedded Python script to check if dataview exists
python3 << EOF
import requests
import json
import sys
...
try:
    # Check if dataview exists using GET _data_view API
    response = requests.get(f"{es_url}/_data_view/{dataview_name}", headers=headers, timeout=30)
    
    if response.status_code == 200:
        print(f"✓ Dataview '{dataview_name}' exists")
        sys.exit(0)
    elif response.status_code == 404:
        print(f"✗ Dataview '{dataview_name}' does not exist")
        sys.exit(1)
    else:
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Dataview check completed successfully - dataview exists"
    exit 0
else
    echo "Fail - Dataview serverless-demo-dataview does not exist"
   fail-message "Dataview serverless-demo-dataview does not exist"
fi 