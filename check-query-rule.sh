...
# Script to check if an Elasticsearch query rule exists using embedded Python
...
RULE_ID="orlando"
echo "Checking if query rule '$RULE_ID' exists..."
...
# Embedded Python script to check if query rule exists
python3 << EOF
import requests
import json
import sys
...
try:
    # Check if query rule exists using GET _query_rules API
    response = requests.get(f"{es_url}/_query_rules/{rule_id}", headers=headers, timeout=30)
    
    if response.status_code == 200:
        print(f"✓ Query rule '{rule_id}' exists")
        sys.exit(0)
    elif response.status_code == 404:
        print(f"✗ Query rule '{rule_id}' does not exist")
        sys.exit(1)
    else:
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Query rule check completed successfully - rule exists"
    exit 0
else
    echo "Fail - Query rule orlando does not exist"
    fail-message "Query rule orlando does not exist"
fi 