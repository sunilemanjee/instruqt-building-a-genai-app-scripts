...
try:
    ...
    response = requests.get(f"{es_url}/_inference/text_embedding/{endpoint_name}", headers=headers, timeout=30)
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "Fail - Inference endpoint my-e5-endpoint does not exist"
    fail-message "Inference endpoint my-e5-endpoint does not exist"
fi 