...
# Script to check if an Elasticsearch inference endpoint exists using embedded Python
...
ENDPOINT_NAME="my-elser-endpoint"
...
# Embedded Python script to check if inference endpoint exists
python3 << EOF
import requests
import json
import sys
...
try:
    # Check if inference endpoint exists using GET _inference API
    response = requests.get(f"{es_url}/_inference/sparse_embedding/{endpoint_name}", headers=headers, timeout=30)
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Inference endpoint check completed successfully - endpoint exists"
    exit 0
else
    echo "Fail - Inference endpoint my-elser-endpoint does not exist"
    fail-message "Inference endpoint my-elser-endpoint does not exist"
fi 